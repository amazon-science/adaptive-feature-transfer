...
class KernelPrior(Prior, nn.Module):
    def __init__(self, feature_dataset, prec, learn_scales=False, tensor_product=False, kernel='linear'):
        Prior.__init__(self)
...
def get_prior(feat_dim, feature_dataset, prec, learn_scales, tensor_product, prior_type):
    assert prior_type in ['kernel', 'kernel_rbf', 'feature', 'kd'], f'Unknown prior type {prior_type}'
...
def get_btune_prior(model_class, dataset, feature_dataset, prec, train_frac):
    ...
    downstream_feat_path = f'./features/{model_class}_{dataset}.pt'
